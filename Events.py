...
def eventHandler(gamepad, values):
    ...
    gamepad.emit(uinput.ABS_THUMBL, values[2], syn=False)

    # Right Joystick
    gamepad.emit(uinput.ABS_RX, values[3], syn=False)
    gamepad.emit(uinput.ABS_RY, values[4], syn=False)
    gamepad.emit(uinput.ABS_THUMBR, values[5], syn=False)
    ...
    gamepad.emit(uinput.BTN_DPAD_RIGHT, values[13], syn=True)